...
## Plik: `src/components/PlanDay.ts`
...
function calcDayCalories(plan: DayPlan): { total: number; incomplete: boolean } {
  let total = 0;
  let incomplete = false;
...
function renderUI() {
  ...
  mount.innerHTML = `
    <div class="max-w-[680px] mx-auto">
      ...
      <!-- Sloty -->
      <ul class="divide-y divide-kreska" id="slots-list">
...
function renderSlot(slot: PlanSlot): string {
  const slotCal = calcSlotCalories(slot);
  const isEmpty = slot.dishes.length === 0;
  const label = SLOT_LABELS[slot.name] ?? slot.name;
  const base = getBaseUrl();

  return `
    <li id="slot-${slot.id}" class="px-4 py-4 transition-all">
      <!-- Nagłówek slotu -->
      <div class="flex items-center justify-between mb-2">
        ...
        <button
          ...
          class="btn-remove-slot text-[12.5px] text-ink-3 min-h-touch px-2"
...
## Lista kontrolna E-03

- [ ] Dzień wyświetlony po polsku, z dniem tygodnia
- [ ] Slot pusty — wyróżniony ramką z linią przerywaną
- [ ] Slot z daniami — lista z kalorami i menu `⋯`
- [ ] Menu dania: „Podmień przepis" (→ E-04) i „Usuń z planu"
- [ ] Usunięcie dania — toast z cofnięciem (5 sekund)
- [ ] „Dodaj kolejne danie" widoczne w każdym niepustym slocie
- [ ] Suma kalorii slotu widoczna przy nazwie posiłku
- [ ] Suma kalorii dnia — na górze, z ostrzeżeniem o niepełności
- [ ] Znacznik `⚠ suma niepełna` — przy posiłku i sumie dnia
- [ ] Dodaj nowy posiłek — wybór z dostępnych SlotTag
- [ ] Usuń posiłek z daniami — ostrzeżenie z liczbą dań
- [ ] Q-10: zmiana dnia przy niepustym planie — dialog ostrzeżenia
- [ ] Powrót z E-04 — scroll do slotu i podświetlenie (przez URL hash)
- [ ] Przycisk „Zrób listę zakupów" — widoczny gdy cokolwiek zaplanowane
- [ ] Responsywne: ≥600px dwa kolumny slotów
